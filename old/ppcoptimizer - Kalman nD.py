...
def main():
    ...
    ############################################ Data prep ###########################################################
    df = pd.read_csv('C:/Users/jtran/Google Drive/growth scalers/Sonoma linen/Sponsored Products Targeting report.csv',
    ...
    for node_col in node_cols:
        BTNode = BTNode.append(RF_preproc.reset_index().groupby([pd.Grouper(freq='1D', key='Date'), node_col])[
    ...
    nday = 30
    r = np.abs(1 / M_t['Clicks'] * M_t['Conversions'] / M_t['Clicks'] * (M_t['Clicks'] - M_t['Conversions']) / M_t[
        'Clicks'])  # =σ^2
    r = r.fillna(0.9)
    print(r)
    ...
    for day in BTNode.loc[lastdayfrom:pd.to_datetime(
            '12/01/2020')].index.unique():  # pd.to_datetime("today")].index.unique(): #for every day
        print(day)
        nday = 14
        ...
        kf.predict()
        r = np.abs(
            nday / M_t['Clicks'] * M_t['Conversions'] / M_t['Clicks'] * (M_t['Clicks'] - M_t['Conversions']) / M_t[
                'Clicks'])
        r = r.fillna(0.1)
        R_t = pd.DataFrame(data=np.diag(r), index=X_t.dropna().index, columns=X_t.dropna().index)
