...
def main():
    ...
    for global_var.path in paths:
        ...
        if 'SP' in campaign_types:
            df_history = df_history_SP[
                ['Date', 'Campaign Name', 'Ad Group Name', 'Targeting', 'Match Type', 'Clicks', 'Conversions', 'Sales']]
        ...
        if 'SB' in campaign_types:
            df_history = pd.concat([df_history, df_history_SB[
                ['Date', 'Campaign Name', 'Ad Group Name', 'Targeting', 'Match Type', 'Clicks', 'Conversions',
                 'Sales']]])
        if 'SBV' in campaign_types:
            df_history = pd.concat([df_history, df_history_SBV[
                ['Date', 'Campaign Name', 'Ad Group Name', 'Targeting', 'Match Type', 'Clicks', 'Conversions',
                 'Sales']]])

        if 'SB' in campaign_types and 'SBV' in campaign_types:
            df_history_SB = pd.concat([df_history_SB, df_history_SBV[
                ['Date', 'Campaign Name', 'Ad Group Name', 'Targeting', 'Match Type', 'Clicks', 'Conversions',
                 'Sales']]])

        try:
            df_history_SB = merge_no_date(df_bid_SB, df_history_SB, 'SB',
                                          global_var.path)  # overwrite df_history without dupplicated label
        except FileNotFoundError:
            print('no SB')
        except ValueError:
            print('no SB')
        ...
        if 'SD' in campaign_types:
            df_history = pd.concat([df_history, df_history_SD[
                ['Date', 'Campaign Name', 'Ad Group Name', 'Targeting', 'Match Type', 'Clicks', 'Conversions',
                 'Sales']]])
...
def merge_no_date(df_bid, df_history, campaign_type, path):
    ...
    if campaign_type == 'SP':
        df_bid_part = dataframe.get_biddable_object(df_bid).loc[:,
                      ['Campaign Name', 'Ad Group Name', 'Targeting', 'Match Type']]  # .drop_duplicates()
    ...
    if campaign_type == 'SD':
        df_bid_part = dataframe.get_biddable_object(df_bid).loc[:,
                      ['Campaign Name', 'Ad Group Name', 'Targeting']]  # .drop_duplicates()
...
def get_slope_conv_value(df_history, df_bid, df_bid_history, path):
    ...
    for i, cam in dfcam.iterrows():  # we could use itertuples to speed up the performance but we would need to have the same format for all campaigns
        ...
        c = object_structure.Campaign(cam['Campaign Id'], cam['Campaign Name'], cam['State'], df_campaign_history,
        ...
        if dfcam['Product'].iloc[0] == 'Sponsored Brands':
            try:
                adgroupID = dataframe.select_row_by_val(df_bid, 'Entity', "Keyword", 'Campaign Id', str(c.campaign_id))[
                    'Ad Group Id (Read only)'].iloc[0]
            except:
                adgroupID = \
                dataframe.select_row_by_val(df_bid, 'Entity', "Product Targeting", 'Campaign Id', str(c.campaign_id))[
                    'Ad Group Id (Read only)'].iloc[0]
            dfadgr = pd.DataFrame({'Ad Group Id': adgroupID, 'Ad Group Name': ['Not available'], 'State': ['enable'],
                                   'Ad Group Default Bid': [0.7]})

        for j, adgr in dfadgr.iterrows():
            df_adgroup_history = dataframe.select_row_by_val(df_history, 'Campaign Name', c.campaign_name,
                                                             'Ad Group Name', adgr['Ad Group Name'])
            df_adgroup_bid_history = dataframe.select_row_by_val(df_bid_history, 'Campaign Id', str(c.campaign_id),
                                                                 'Ad Group Id', str(adgr['Ad Group Id']))
            ...
            dftarget = dataframe.select_row_by_val(dftarget, 'Campaign Id', str(c.campaign_id), 'Ad Group Id',
                                                   str(a.adgroup_id))
    ...
    df_bid = pd.merge(df_bid, dftarget, how='left', left_on='Campaign Id', right_on='Campaign Id').drop_duplicates(
        ignore_index=True)
    print(global_var.path)
