...
def main():
    ...
    SKUs = ASINs.join(
        dataframe.select_row_by_val(df_list, 'fulfillment-channel', 'DEFAULT')[['asin1', 'seller-sku']].set_index(
            'asin1'))  # , rsuffix='_FBM')
    SKUs.rename(columns={'seller-sku': 'seller-sku_FBM'}, inplace=True)
    print(SKUs)
    SKUs = SKUs.join(
        dataframe.select_row_by_val(df_list, 'fulfillment-channel', 'AMAZON_NA')[['asin1', 'seller-sku']].set_index(
            'asin1'))  # , rsuffix='_FBA')
