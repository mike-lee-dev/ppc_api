...
class _CampaignBase:
	...
	def get_slope(self, df_bid_history):
		...
		if (count_clicks) <=4:
			...
			try:
				print(dataframe.select_row_by_val(dataframe.get_biddable_object(df_bid_history),'Campaign Id',self.campaign_id).dropna(subset=['avg_cpc']))
				self.a=market_curve.market_curve(dataframe.select_row_by_val(dataframe.get_biddable_object(df_bid_history),'Campaign Id',self.campaign_id).dropna(subset=['avg_cpc']))
	...
	def get_conversion_value(self, df_history, df_bid_history):
		if df_bid_history['Product'].iloc[0]=='Sponsored Brands':
			...
			try:
				listing=input_output.active_listing(global_var.path)
	...
	def get_conversion_value(self, df_history, df_bid_history):
		...
		# 	return
			...
			if df_bid_history['Product'].iloc[0]=='Sponsored Brands':
				...
				SKUs=dataframe.select_row_by_val(df_bid_history,'Entity','Product Ad')
				prices=input_output.active_listing(global_var.path)
				print(prices)
...
class _TargetBase():#(_AdgroupBase):
	...
	def get_conv_rate():
		...
		#cond=df['Campaign Id']==self.campaign_id & df['Ad Group Name']==self.adgroup_name & df['Targeting']==self.targeting & df['Match Type'] == self.match_type
		cond=df['Campaign Id']==self.campaign_id & df['Ad Group Id']==self.adgroup_id & df['Targeting']==self.targeting & df['Match Type'] == self.match_type
