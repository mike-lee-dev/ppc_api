...
class _CampaignBase:
    ...
    def get_slope(self, df_bid_history):
        ...
        if (count_clicks) <= 4:
            ...
            try:
                print(dataframe.select_row_by_val(dataframe.get_biddable_object(df_bid_history), 'Campaign Id',
                                                  self.campaign_id).dropna(subset=['avg_cpc']))
                self.a = market_curve.market_curve(
                    dataframe.select_row_by_val(dataframe.get_biddable_object(df_bid_history), 'Campaign Id',
                                                self.campaign_id).dropna(subset=['avg_cpc']))
...
class _TargetBase():  # (_AdgroupBase):
    ...
    def get_conv_rate():
        ...
        cond = df['Campaign Id'] == self.campaign_id & df['Ad Group Id'] == self.adgroup_id & df[
