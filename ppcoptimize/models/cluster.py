...
def resample(RF_preproc):
    ...
    RF_preproc = RF_preproc.groupby(
        [pd.Grouper(freq='W'), 'Campaign_Name', 'Ad_Group_Name', 'Targeting', 'Match_Type']).sum().reset_index()
    return RF_preproc
...
def encoding(RF_preproc):
    ...
    RF_preproc['Campaign_Name_enc'] = lecampaign.fit_transform(RF_preproc['Campaign_Name'])
    RF_preproc['Ad_Group_Name_enc'] = leadgroup.fit_transform(RF_preproc['Ad_Group_Name'])
    RF_preproc['Targeting_enc'] = letargeting.fit_transform(RF_preproc['Targeting'])  # -> This can lead to overfitting
    RF_preproc['Match_Type_enc'] = lematchtype.fit_transform(RF_preproc['Match_Type'])
...
def feature_selection(RF_preproc):
    ...
    # to_keep = ['Portfolio_name_enc','Campaign_Name_enc','Ad_Group_Name_enc','Targeting_enc','Match_Type_enc', 'Number_of_word', 'Clicks','Conversions']
    to_keep = ['Campaign_Name_enc', 'Ad_Group_Name_enc', 'Targeting_enc', 'Match_Type_enc', 'Number_of_word', 'Clicks',
               'Conversions']
...
def decoding_node(RF_preproc, lecampaign, leadgroup, letargeting, lematchtype):
    ...
    RF_decoding['Campaign_Name'] = lecampaign.inverse_transform(RF_decoding['Campaign_Name_enc'])
    RF_decoding['Ad_Group_Name'] = leadgroup.inverse_transform(RF_decoding['Ad_Group_Name_enc'])
    RF_decoding['Targeting'] = letargeting.inverse_transform(RF_decoding['Targeting_enc'])
    RF_decoding['Match_Type'] = lematchtype.inverse_transform(RF_decoding['Match_Type_enc'])
    # RF_decoding=RF_decoding[['Leave','Portfolio_name', 'Campaign_Name', 'Ad_Group_Name', 'Targeting', 'Match_Type']]
    RF_decoding = RF_decoding[['Leave', 'Campaign_Name', 'Ad_Group_Name', 'Targeting', 'Match_Type']]
    RF_decoding = RF_decoding.drop_duplicates(subset=['Campaign_Name', 'Ad_Group_Name', 'Targeting', 'Match_Type'])
    print(f"decoding: \n{RF_decoding}")
    return RF_decoding
...
def aggregate_by_node(RF_decoding, df):
    ...
    RF_decoding_copy.columns = [c.replace('_', ' ') for c in
                                RF_decoding_copy.columns]  ####This would only work if the name has no_
    dfclustered = df.merge(RF_decoding_copy,
                           on=['Campaign Name', 'Ad Group Name', 'Targeting', 'Match Type'],
                           # right_on=['Campaign_Name', 'Ad_Group_Name','Targeting','Match_Type'],
                           how='left').drop_duplicates(
        subset=['Date', 'Campaign Name', 'Ad Group Name', 'Targeting', 'Match Type'], ignore_index=True)
    dfclustered = dfclustered.groupby(by=['Date', 'Leave'], as_index=False).agg(
        {'Clicks': 'sum', 'Conversions': 'sum', 'Sales': 'sum'}).set_index('Date')
