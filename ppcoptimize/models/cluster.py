...
def resample(RF_preproc):
	...
	RF_preproc=RF_preproc.groupby([pd.Grouper(freq='W'),'Campaign_Name','Ad_Group_Name','Targeting','Match_Type']).sum().reset_index()
	return RF_preproc
...
def feature_selection(RF_preproc):
	...
	to_keep = ['Campaign_Name_enc','Ad_Group_Name_enc','Targeting_enc','Match_Type_enc', 'Number_of_word', 'Clicks','Conversions']
...
def aggregate_by_node(RF_decoding, df):
	...
	dfclustered=df.merge(RF_decoding_copy, 
		...
		how='left').drop_duplicates(subset=['Date','Campaign Name','Ad Group Name','Targeting','Match Type'],ignore_index=True)
