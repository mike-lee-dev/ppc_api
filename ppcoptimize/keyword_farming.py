...
def main():
    ...
    for i, row in df_search_term_filtered.iterrows():
        ...
        adgroup_dst_ids = \
        dataframe.select_row_by_val(linkdf, 'Campaign source Name', auto_campaign_name, 'Ad Group source name',
                                    auto_adgroup_name)['Ad Group destination Id']
        auto_campaign_id = \
        dataframe.select_row_by_val(linkdf, 'Campaign source Name', auto_campaign_name, 'Ad Group source name',
                                    auto_adgroup_name)['Campaign source Id'].iloc[0]
        auto_adgroup_id = \
        dataframe.select_row_by_val(linkdf, 'Campaign source Name', auto_campaign_name, 'Ad Group source name',
                                    auto_adgroup_name)['Ad Group source Id'].iloc[0]
        for adgroup_dst_id in adgroup_dst_ids:
            campaign_dst_id = dataframe.select_row_by_val(linkdf, 'Ad Group destination Id', adgroup_dst_id)[
                'Campaign destination Id'].iloc[0]
            df_adgroup_dst = dataframe.select_row_by_val(df_bid_SP, 'Campaign Id', campaign_dst_id, 'Ad Group Id',
                                                         adgroup_dst_id)
            ...
            if re.match('^b0', row['Customer Search Term']) and match_type_dst == 'PAT':
                ...
                campaign_management.create_keyword_pat('Sponsored Products', entity, campaign_dst_id, adgroup_dst_id,
                                                       '', pat, match_type_dst, bid, path)
                entity = 'Negative Product Targeting'
                campaign_management.create_keyword_pat('Sponsored Products', entity, auto_campaign_id, auto_adgroup_id,
                                                       '', pat, '-', '', path)
            ...
            if not re.match('^b0', row['Customer Search Term']) and match_type_dst == 'mix':
                ...
                entity = 'Keyword'
                campaign_management.create_keyword_pat('Sponsored Products', entity, campaign_dst_id, adgroup_dst_id,
                                                       targeting, '', match_type_dst, bid, path)
                entity = 'Negative Keyword'
                campaign_management.create_keyword_pat('Sponsored Products', entity, auto_campaign_id, auto_adgroup_id,
                                                       targeting, '', 'negativePhrase', '', path)
