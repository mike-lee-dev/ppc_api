...
def read_bid(campaign_type, path_bid, filename='bulk.xlsx'):
	...
	if campaign_type=='SP':
		df_bid = pd.read_excel(io=path,sheet_name=ws_name,header=0, engine="openpyxl", converters={'Campaign Id':str, 'Ad Group Id': str,'Keyword Id (Read only)':str, 'Product Targeting Id (Read only)':str})	#  index_col=0)
		df_bid['Campaign Type']=campaign_type
		df_bid=dataframe.get_cpc(df_bid)
		df_bid['Targeting']=df_bid.apply(lambda x: x['Keyword Text'] if x['Entity']=='Keyword' else  x['Resolved Product Targeting Expression (Informational only)'], axis=1)
		df_bid['Keyword Id (Read only)']=df_bid['Keyword Id (Read only)'].fillna(0)#.astype('Int64').astype('str')
		df_bid['Product Targeting Id (Read only)']=df_bid['Product Targeting Id (Read only)'].fillna(0)#.astype('Int64').astype('str')
		df_bid['Campaign Name']=df_bid['Campaign Name (Informational only)']
		df_bid['Ad Group Name']=df_bid['Ad Group Name (Informational only)']
		df_bid['Target Id']=df_bid.apply(lambda x: x['Keyword Id (Read only)'] if x['Entity']=='Keyword' else  x['Product Targeting Id (Read only)'], axis=1)
	
	if campaign_type=='SB' or  campaign_type=='SBV':
		df_bid = pd.read_excel(io=path,sheet_name=ws_name,header=0, engine="openpyxl", converters={'Campaign Id':str, 'Ad Group Id (Read only)': str,'Keyword Id (Read only)':str, 'Product Targeting Id (Read only)':str})	#  index_col=0)
		df_bid['Campaign Type']=campaign_type
		df_bid=dataframe.get_cpc(df_bid)
		df_bid['Targeting']=df_bid.apply(lambda x: x['Keyword Text'] if x['Entity']=='Keyword' else  x['Product Targeting Expression'], axis=1)
		df_bid['Keyword Id (Read only)']=df_bid['Keyword Id (Read only)'].fillna(0)#.astype('Int64').astype('str')
		df_bid['Product Targeting Id (Read only)']=df_bid['Product Targeting Id (Read only)'].fillna(0)#.astype('Int64').astype('str')
		df_bid['Campaign Name']=df_bid['Campaign Name (Informational only)']
	...
	if campaign_type=='SD':
		df_bid = pd.read_excel(io=path,sheet_name=ws_name,header=0, engine="openpyxl", converters={'Campaign Id':str, 'Ad Group Id': str,'Targeting Id (Read only)':str})	#  index_col=0)
		df_bid['Campaign Type']=campaign_type
		df_bid=dataframe.get_cpc(df_bid)
	...
	df_bid['Match Type']=df_bid.apply(lambda x: '-' if x['Entity']=='Product Targeting' else  x['Match Type'], axis=1)
	return df_bid
...
def read_bid_history(campaign_type, path):
	...
	if campaign_type =='SP':
		df['Bid']=df.apply(lambda x: x['Bid'] if x['Bid']>0 else  x['Ad Group Default Bid (Informational only)'], axis=1)

	if campaign_type =='SB':
		df['Ad Group Name']='Not available'
	if campaign_type =='SD':
		df['Match Type']="-"
		df['Bid']=df.apply(lambda x: x['Ad Group Default Bid (Informational only)'] if x['Bid']=='' else  x['Bid'], axis=1)
...
def read_report(campaign_type, path):
	...
	if campaign_type=='SP' or  campaign_type=='SB' or  campaign_type=='SBV':
		...
		df=dataframe.change_val_if_col_contains(df,'Targeting','loose-match','Match Type','Targeting Expression Predefined')
		df=dataframe.change_val_if_col_contains(df,'Targeting','complements','Match Type','Targeting Expression Predefined')
		df=dataframe.change_val_if_col_contains(df,'Targeting','substitutes','Match Type','Targeting Expression Predefined')
		df=dataframe.change_val_if_col_contains(df,'Targeting','close-match','Match Type','Targeting Expression Predefined')
	if campaign_type=='SP':
		try:
...
def read_link(path):
	...
	df=pd.read_excel(path_file,header=0, engine="openpyxl", converters={'Ad Group source Id':str, 'Ad Group destination Id':str})
