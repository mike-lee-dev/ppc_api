...
def get_product_by_adgroup(df_bid, adgroup_id):
    productIDs = \
    dataframe.select_row_by_val(df_bid, 'Ad Group Id', adgroup_id, 'Entity', 'Product Ad', 'State', 'enabled')[
        'SKU'].to_numpy()
    return productIDs


def get_matchtype_by_adgroup(df_bid, adgroup_id):
    match_types = \
    dataframe.select_row_by_val(df_bid, 'Ad Group Id', adgroup_id, 'Entity', 'Keyword', 'State', 'enabled')[
        'Match Type'].unique()
    return match_types
...
def campaign_link_manual(path_bid):
    ...
    for df_bid in list(
            combinations([df_bid_manual, df_bid_SB, df_bid_SD], 2)):  # [('SP', 'SB'), ('SP', 'SD'), ('SB', 'SD')]
        if df_bid[0]['Product'].iloc[0] == 'Sponsored Brands':
            adgroup_src_ids = dataframe.select_row_by_val(df_bid[0], 'Campaign State (Informational only)', 'enabled')[
                'Ad Group Id'].unique()
        else:
            adgroup_src_ids = dataframe.select_row_by_val(df_bid[0], 'Ad Group State (Informational only)', 'enabled')[
                'Ad Group Id'].unique()
        for adgroup_src_id in adgroup_src_ids:
            ...
            if 'defen' in campaign_src_name.lower():
                continue
            if 'catch' in campaign_src_name.lower():
                continue
            if 'competi' in campaign_src_name.lower():
                continue
            if 'cat' in campaign_src_name.lower():
                continue
            if 'brand' in campaign_src_name.lower():
                continue
            if 'defen' in adgroup_src_name.lower():
                continue
            if 'catch' in adgroup_src_name.lower():
                continue
            if 'competi' in adgroup_src_name.lower():
                continue
            if 'cat' in adgroup_src_name.lower():
                continue
            if 'brand' in adgroup_src_name.lower():
                continue
            if df_bid[1]['Product'].iloc[0] == 'Sponsored Brands':
                adgroup_dst_ids = \
                dataframe.select_row_by_val(df_bid[1], 'Campaign State (Informational only)', 'enabled')[
                    'Ad Group Id'].unique()

            else:
                print(df_bid[1])
                adgroup_dst_ids = \
                dataframe.select_row_by_val(df_bid[1], 'Ad Group State (Informational only)', 'enabled')[
                    'Ad Group Id'].unique()
            for adgroup_dst_id in adgroup_dst_ids:
                campaign_dst_name = dataframe.select_row_by_val(df_bid[1], 'Ad Group Id', adgroup_dst_id)[
                    'Campaign Name (Informational only)'].iloc[0]
                campaign_dst_id = \
                dataframe.select_row_by_val(df_bid[1], 'Ad Group Id', adgroup_dst_id)['Campaign Id'].iloc[0]
                campaign_dst_type = \
                dataframe.select_row_by_val(df_bid[1], 'Ad Group Id', adgroup_dst_id)['Product'].iloc[0]
                if df_bid[1]['Product'].iloc[0] == 'Sponsored Brands':
                    adgroup_dst_name = 'Not available'
                else:
                    adgroup_dst_name = dataframe.select_row_by_val(df_bid[1], 'Ad Group Id', adgroup_dst_id)[
                        'Ad Group Name (Informational only)'].iloc[0]
            ...
            if best_score != 0.:
                ...
                campaign_management.create_adgroup_link(campaign_src_name, adgroup_src_name, campaign_src_id,
                                                        adgroup_src_id, campaign_src_type,
                                                        best_campaign_dst_name, best_adgroup_dst_name,
                                                        best_campaign_dst_id, best_adgroup_dst_id,
                                                        best_campaign_src_type, path_bid, '-manual-manual'
                                                        )
...
def campaign_link_auto(path):
    ...
    adgroup_src_ids = dataframe.select_row_by_val(df_bid_auto, 'Ad Group State (Informational only)', 'enabled')[
        'Ad Group Id'].unique()
    ...
    for adgroup_src_id in adgroup_src_ids:
        ...
        best_campaign_dst_id = ''
        best_adgroup_dst_id = ''
        campaign_src_name = dataframe.select_row_by_val(df_bid_auto, 'Ad Group Id', adgroup_src_id)[
            'Campaign Name (Informational only)'].iloc[0]
        ...
        adgroup_src_name = dataframe.select_row_by_val(df_bid_auto, 'Ad Group Id', adgroup_src_id)[
            'Ad Group Name (Informational only)'].iloc[0]
        ...
        adgroup_dst_ids = dataframe.select_row_by_val(df_bid_manual, 'Ad Group State (Informational only)', 'enabled')[
            'Ad Group Id'].unique()
        for adgroup_dst_id in adgroup_dst_ids:
            campaign_dst_name = dataframe.select_row_by_val(df_bid_manual, 'Ad Group Id', adgroup_dst_id)[
                'Campaign Name (Informational only)'].iloc[0]
            if 'defen' in campaign_dst_name.lower():
                continue
            if 'catch' in campaign_dst_name.lower():
            ...
            if 'brand' in campaign_dst_name.lower():
                continue
            campaign_dst_id = \
            dataframe.select_row_by_val(df_bid_manual, 'Ad Group Id', adgroup_dst_id)['Campaign Id'].iloc[0]
            campaign_dst_type = \
            dataframe.select_row_by_val(df_bid_manual, 'Ad Group Id', adgroup_dst_id)['Product'].iloc[0]
            adgroup_dst_name = dataframe.select_row_by_val(df_bid_manual, 'Ad Group Id', adgroup_dst_id)[
                'Ad Group Name (Informational only)'].iloc[0]
            if 'defen' in adgroup_dst_name.lower():
                continue
            if 'catch' in adgroup_dst_name.lower():
            ...
            if new_score > 0.8:
                ...
                campaign_management.create_adgroup_link(campaign_src_name, adgroup_src_name, campaign_src_id,
                                                        adgroup_src_id, campaign_src_type,
                                                        campaign_dst_name, adgroup_dst_name, campaign_dst_id,
                                                        adgroup_dst_id, campaign_dst_type, path, '-auto-manual'
                                                        )
