...
def main():
	...
	_df_no_clicks=df_no_clicks[['Date','Portfolio name','Campaign Name','Ad Group Name','Targeting', 'Match Type']].copy()
	print(df_history)
	...
	df_bid=pd.merge(df_bid,df_forecast, how='left', left_on=['Campaign', 'Ad Group','Keyword or Product Targeting', 'Match Type'], right_on=['Campaign_Name', 'Ad_Group_Name','Targeting', 'Match_Type'])
	print(df_forecast)
	print(df_bid)
	...
	for i, cam in dfcam.iterrows(): # we could use itertuples to speed up the performance but we would need to have the same format for all campaigns 
		...
		c=object_structure.Campaign(cam['Campaign ID'],cam['Campaign'],cam['Campaign Status'], df_campaign_history, df_campaign_bid_history,1.,default_conv_val)
		print(c.a)
		...
		for j, adgr in dfadgr.iterrows():
			...
			df_adgroup_history=dataframe.select_row_by_val(df_history,'Campaign Name', c.campaign_name, 'Ad Group Name',adgr['Ad Group'])
			df_adgroup_bid_history=dataframe.select_row_by_val(df_bid_history,'Campaign', c.campaign_name, 'Ad Group',adgr['Ad Group'])
			...
			for k, target in dftarget.iterrows():
				...
				if target['Match Type'] in ['Targeting Expression', 'Targeting Expression Predefined', 'broad', 'phrase', 'exact']:
					...
					dataframe.change_val_by_cond(
						df_bid,'a', t.a, 'Campaign ID',c.campaign_id, 'Ad Group', a.adgroup_name, 'Keyword or Product Targeting', t.target_name,
						'Match Type',t.match_type
						)
					dataframe.change_val_by_cond(
						df_bid,'conv_value', t.conv_value, 'Campaign ID',c.campaign_id, 'Ad Group', a.adgroup_name, 'Keyword or Product Targeting', t.target_name,
						'Match Type',t.match_type
						)
					print(t.conv_value)
	...
	for row in ws.iter_rows(min_row=2):
		...
		if oldbid!="" and (recordtype=='Keyword' or recordtype=='Product Targeting'):
			...
			newbid=dataframe.select_row_by_val(df_bid, 'Campaign', campaign, 'Ad Group', adgroup,'Keyword or Product Targeting', target, 'Match Type', matchtype)['new_bid'].values[0]
			optimized=dataframe.select_row_by_val(df_bid, 'Campaign', campaign, 'Ad Group', adgroup,'Keyword or Product Targeting', target, 'Match Type', matchtype)['optimized'].values[0]
			if optimized:
				print(f"new bid: {str(round(newbid,2))}")
				print(f"row: {row[0].row}")
				print(f"Could not find bid for campaign: {campaign}, adgroup: {adgroup}, target: {target}, matchtype: {matchtype}")
