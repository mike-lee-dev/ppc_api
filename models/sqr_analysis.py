...
def phrase_match_suggestion():
	...
	data = pd.DataFrame(dataset[['Customer Search Term', 'Impressions', 'Clicks', 'Spend', '7 Day Total Sales ', '7 Day Total Orders (#)']])
	...
	final_matching_dict = {}
	for key, val in matching_dict.items():
		final_matching_dict[key] = [sum(new_data.iloc[val]['Impressions']),sum(new_data.iloc[val]['Clicks']), sum(new_data.iloc[val]['Spend']), sum(new_data.iloc[val]['7 Day Total Sales ']),sum(new_data.iloc[val]['7 Day Total Orders (#)']), list(new_data.iloc[val]['Customer Search Term'] )]
		final_matching_dict[key] = [round(elem, 2) if isinstance(elem, int) else elem for elem in final_matching_dict[key] ]
		#print(final_matching_dict)
	Final_results = pd.DataFrame.from_dict(final_matching_dict, orient = 'index')
	Final_results.columns = ['Impressions', 'Clicks', 'Spend', '7 Day Total Sales', '7 Day Total Orders (#)', 'Customer Search Terms']
	# Save to excel file
	...
	df_merge=pd.merge(df_merge,df_bid_SP,left_on='Keyword suggestion', right_on='Keyword Text',how="outer",indicator=True)
...
def exact_match_suggestion():
	...
	df_merge=pd.merge(df_merge,df_bid_SP,left_on='Customer Search Term', right_on='Keyword Text',how="outer",indicator=True)
	df_merge=df_merge[df_merge['_merge']=='left_only']
	print(df_merge)
	return df_merge
