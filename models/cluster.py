...
def resample(RF_preproc):
    ...
    RF_preproc = RF_preproc.groupby(
        [pd.Grouper(freq='W'), 'campaignName', 'adGroupName', 'targeting', 'matchType']).sum().reset_index()
    return RF_preproc
...
def feature_selection(RF_preproc):
    ...
    to_keep = ['campaignName_enc', 'adGroupName_enc', 'targeting_enc', 'matchType_enc', 'Number_of_word', 'clicks',
               'conversions']
...
def aggregate_by_node(RF_decoding, df):
    ...
    dfclustered = df.merge(RF_decoding_copy,
        ...
        subset=['date', 'campaignName', 'adGroupName', 'targeting', 'matchType'], ignore_index=True)
