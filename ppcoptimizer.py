...
def get_slope_conv_value(df_campaign, df_history, df_bid, df_bid_history, account):
	...
	for i, cam in df_campaign.iterrows(): # we could use itertuples to speed up the performance but we would need to have the same format for all campaigns 
		...
		c=object_structure.Campaign(cam['campaignId'],cam['campaignName'],cam['State'], df_campaign_history, df_campaign_bid_history,1.,default_conv_val)
		...
		for j, adgr in dfadgr.iterrows():
			df_adgroup_history=dataframe.select_row_by_val(df_history,'campaignName', c.campaign_name, 'adGroupName',adgr['adGroupName'])
			df_adgroup_bid_history=dataframe.select_row_by_val(df_bid_history,'campaignId', str(c.campaign_id), 'adGroupId',str(adgr['adGroupId']))
			...
			dftarget=dataframe.select_row_by_val(dftarget, 'campaignId', str(c.campaign_id), 'adGroupId', str(a.adgroup_id))
