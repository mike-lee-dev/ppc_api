...
def main():
	...
	for i, row in linkdf.iterrows():
		...
		if row['Campaign destination type']=='Sponsored Display':
			dfdst=df_bid_SD		
		df_adgr_src=dataframe.select_row_by_val(dfsrc,'Ad Group Id', str(row['Ad Group source Id']),'State','enabled')#,'Campaign State (Informational only)','enabled','Ad Group State (Informational only)','enabled')
		df_adgr_dest=dataframe.select_row_by_val(dfdst,'Ad Group Id',str(row['Ad Group destination Id']),'State','enabled')		
		campaign_management.add_missing_keyword(df_adgr_src, df_adgr_dest, str(row['Ad Group source Id']), str(row['Ad Group destination Id']), path)
